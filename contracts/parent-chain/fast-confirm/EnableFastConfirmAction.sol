...
contract EnableFastConfirmAction {
  ...
  constructor(
    ...
    address gnosisSafe1_3_0,
    address gnosisCompatibilityFallbackHandler
  ) {
    require(
      gnosisSafeProxyFactory.code.length > 0,
      "gnosisSafeProxyFactory doesn't exist on this chain"
    );
    require(
      gnosisSafe1_3_0.code.length > 0,
      "gnosisSafe1_3_0 doesn't exist on this chain"
    );
    require(
  ...
  function perform(
    IRollupAdmin rollup,
    address[] calldata fastConfirmCommittee,
    uint256 threshold,
    uint256 salt
  ) external {
    require(
      rollup.anyTrustFastConfirmer() == address(0),
      'Fast confirm already enabled'
    );
    require(
      threshold > 0 && threshold <= fastConfirmCommittee.length,
      'Invalid threshold'
    );
    for (uint256 i = 0; i < fastConfirmCommittee.length; i++) {
      require(fastConfirmCommittee[i] != address(0), 'Invalid address');
      require(
        rollup.isValidator(fastConfirmCommittee[i]),
        'fastConfirmCommittee members must be validator'
      );
    }
